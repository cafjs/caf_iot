...
exports.newInstance = function(context, spec, secrets, cb) {
        ...
        var iterF = function(action, cb1) {
            ...
            case 'updateIoT':
                if (iots[action.deviceId]) {
                    ...
                    $.log && $.log.debug("Ignoring: cannot update: not added" +
                                         "IoT device " + action.deviceId);
                    cb1(null);
                }
                break;
            default:
    ...
    that.getCommandsOutput = function(deviceId) {
         ...
         *
         * {type: "caf_iot.channel", lastModified: number, firstIndex: number,
         * values: Array.<string>}
         *
         */
    ...
    var gcCommands = function(changeSet) {
        ...
        for (var deviceId in iots) {
            ...
            /* Commands type is
             *
             * {type: "caf_iot.channel", lastModified: number,
             *  firstIndex: number,
             * values: Array.<string>}
             *
             */
            ...
            if (lastSeenVersion && lastModifVersion &&
                (lastSeenVersion >=  lastModifVersion) && commands &&
                commands.values) {
    ...
    var applyChanges = function() {
        ...
        /*  Update version of `toCloud` to reflect processed responses
         * by this CA so that the device can GC.*/
