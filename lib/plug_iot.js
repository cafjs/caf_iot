...
'if redis.call("exists", KEYS[1]) == 0 then \
    local st1 = "{\"deviceView\" : true, \"toCloud\" : {\"version\" : 0},\
    \"fromCloud\" : {\"version\" : 0}}"\
    local st2 = "{\"deviceView\" : false, \"toCloud\" : {\"version\" : 0},\
    \"fromCloud\" : {\"version\" : 0}}"\
    return redis.call("hmset", KEYS[1], "device_version", 0, "device_content",\
                      st1, "ca_version", 0, "ca_content", st2);\
else \
    return {ok ="OK"} \
end ';
 ...
 * KEYS deviceId
 * 
 * ARGV[1] 1 if device_content will be read, 0  if
 * ca_content should be read instead.
 * 
 */
var getIoT = 
'if redis.call("exists", KEYS[1]) == 0 then \
...
elseif  ARGV[1] == 1 then \
   return redis.call("hget", KEYS[1], "device_content"); \
elseif  ARGV[1] == 0 then \
   return redis.call("hget", KEYS[1], "ca_content"); \
else \
   return {err ="Error: invalid flag "  .. ARGV[1] .. " should be 0 or 1"} \
end ';


/*
 * Checks whether the current version matches and then resets the IoT device
 * info. It does nothing if the check fails.
 * 
 * KEYS deviceId
 * 
...
'function testAndSet (versionStr, contentStr, readContentStr, key, version, \
                     content) \
...
exports.newInstance = function(context, spec, secrets, cb) {
    ...
    that.routeConfig = function(app, iotUrl) {
        app.post(iotUrl, function(req, res) {
                     ...
                     if (req.body &&  req.body.toCloud && 
                          ...
                          * and we return last device_version/device_content
                          */
    ...
    that.createIoT = function(deviceId, cb0) {
        doLua('createIoT', false, deviceId, null, null, cb0);        
    };
     ...
     * 
     * @param {string} deviceId A globally unique identifier for the device.
     * @param {boolean} isFromDevice True if we want the view from the IoT 
     * device. False if we want the CA view. 
     * @param {caf.cb} cb0 A callback to return error (string) or the state as
     *  a an object of type {deviceView: boolean, 
     *                       toCloud: {version: number, values: Object=}, 
     ...
     * @function
     */
     that.readIoT = function(deviceId, isFromDevice, cb0) {
         var cb1 = function(err, data) {
             if (err) {
                 cb0(err, data);
             } else {
                 cb0(err, JSON.parse(data));
             }
         };
        doLua('getIoT', isFromDevice, deviceId, null, null, cb1);
    };
     ...
     * 
     * @param {string} deviceId A globally unique identifier for the device.
     * @param {number} version The new version for the state.
     ...
     * @function
     */  
    that.updateIoT = function(deviceId, version, content, cb0) {
